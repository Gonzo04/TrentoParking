...
\begin{itemize}
  ...
  \item Link deploy: \url{https://trentoparking-frontend.onrender.com}
  \item Account demo:
  \begin{itemize}
...
\textbf{Nota:} le email transazionali (verifica account, reset password) sono inviate
tramite Mailjet. In assenza di un dominio personalizzato verificato, alcune email
potrebbero essere recapitate nella cartella spam del destinatario.
...
\centering
\begin{tikzpicture}[
  box/.style={rectangle, draw, rounded corners, minimum width=3.6cm,
    minimum height=1cm, align=center, font=\small},
  ext/.style={box, fill=gray!15},
  arr/.style={->, thick}
]
\node[box, fill=blue!10]   (browser)  at (4,9)   {Browser (Client)};
\node[box, fill=green!10]  (frontend) at (4,6.5) {Render Static Site\\React + Vite};
\node[box, fill=orange!10] (backend)  at (4,4)   {Render Web Service\\Express + Node.js};
\node[box, fill=red!10]    (mongo)    at (0.5,1.5) {MongoDB Atlas\\(cloud)};
\node[ext]                 (resend)   at (4,1.5)   {Mailjet API\\(email)};
\node[box, fill=yellow!10] (fs)       at (9.5,4)   {File System\\/uploads (ephemeral)};

\draw[arr] (browser)  -- node[right, font=\scriptsize]{HTTPS} (frontend);
\draw[arr] (frontend) -- node[right, font=\scriptsize]{REST /api/*} (backend);
\draw[arr] (backend)  -- node[left,  font=\scriptsize]{Mongoose} (mongo);
\draw[arr] (backend)  -- node[right, font=\scriptsize]{Mailjet SDK} (resend);
...
\begin{itemize}
  \item \textbf{Frontend}: Static Site su Render
        (\url{https://trentoparking-frontend.onrender.com}). Applicazione React
        compilata con Vite, comunica con il backend tramite chiamate REST
        autenticate con JWT.
  \item \textbf{Backend}: Web Service su Render
        (\url{https://trentoparking-backend.onrender.com}). Server Express che
        espone le API REST, gestisce l'autenticazione JWT, il caricamento
        file tramite Multer e l'invio email tramite Mailjet.
  \item \textbf{Database}: MongoDB Atlas (cloud) con Mongoose come ODM.
        I file caricati vengono salvati nel filesystem del container
        (\texttt{/uploads/}) e serviti come file statici.
\end{itemize}
...
Mailjet      & ---     & Invio email transazionale (verifica account, reset password) \\
...
\subsubsection*{Infrastruttura di deploy}

\begin{table}[h!]
\centering
\begin{tabular}{|l|l|l|}
\hline
\textbf{Servizio} & \textbf{Piano} & \textbf{Utilizzo} \\
\hline
...
Mailjet             & Free & Invio email transazionali \\
\hline
\end{tabular}
\caption{Infrastruttura di deploy}
\end{table}
