...
\node[ext]                 (resend)   at (4,1.5)   {Resend API\\(email)};
...
\draw[arr] (backend)  -- node[right, font=\scriptsize]{Resend SDK} (resend);
...
\begin{itemize}
  \item \textbf{Frontend}: Static Site su Render
        (\texttt{trentoparking-frontend.onrender.com}). Applicazione React
        compilata con Vite, comunica con il backend tramite chiamate REST
        autenticate con JWT.
  \item \textbf{Backend}: Web Service su Render
        (\texttt{trentoparking-backend.onrender.com}). Server Express che
        espone le API REST, gestisce l'autenticazione JWT, il caricamento
        file tramite Multer e l'invio email tramite Resend.
